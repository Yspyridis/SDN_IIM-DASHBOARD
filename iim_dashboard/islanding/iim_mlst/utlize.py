...
def total_Progressive(net,gen_ids,switch_id,features,adj_matrix, number_clusters=None):
    ...
    # #merge between switch
    # for i in range(len(switch_id)):
    #     # from_num = lines[i][2]
    #     # to_num = lines[i][3]
    #     # a = net.res_bus.loc[from_num].values
    #     # b = net.res_bus.loc[to_num].values
    #     # a_index, a_succ = search_1d(a, net.res_bus.values)
    #     # b_index, b_succ = search_1d(b, net.res_bus.values)
    #     current_switch_from= lines[i][2]
    #     current_switch_to= lines[i][3]
    #     if current_switch_from in gen_ids :
    #         out_index=current_switch_to
    #     elif current_switch_to in  gen_ids:
    #         out_index=current_switch_from
    #
    #     #if one connects to generator, use another one
    #     else:
    #     #if non of them are generators, average
...
def weighted_KMeans(data,generator_index, k=2,theta_gen=0.7,max_time_step=100):
    ...
    while not converged:
        ...
        for i in range(n):
            ...
            for j in range(k):
                # if i ==generator_index[j]:
                #
                #     dist = n*theta_gen*_distance(data[i], centroids[j])
                # else:
                #     dist = n*rest_importance*_distance(data[i], centroids[j])
